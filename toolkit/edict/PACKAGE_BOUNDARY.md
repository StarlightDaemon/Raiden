...
This document defines the current high-level boundary of the managed `Edict`
package surface.
...
- what remains intentionally undecided until updater canon resumes

It does not define exact manifest fields or archive formats.

## Core Rule

The `Edict` is the managed core inside a downstream `RAIDEN Instance`.

It should contain the material that RAIDEN owns centrally and expects to update
as a governed unit.
...
These remain conditional until updater-shape canon exists.
...
If the answer to all three is yes, it is a good managed-core candidate.
...
- prepare for later manifest and compatibility specs

without pretending the update mechanism is already decided.
